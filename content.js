...
(() => {
  ...
  function classifyV2(text, postEl) {
    const info = classifyV2Core(text, {
      devopsKeywords: DEVOPS_KEYWORDS,
      hiringSignals: HIRING_SIGNALS,
      invalidKeywords: INVALID_KEYWORDS,
    }, {
      bodyText: postEl ? getPostBodyOnly(postEl) : text,
    });

    if (info.v2signals) {
      dbg('[V2]', info.match ? 'MATCH' : 'SKIP', `confidence=${info.confidence}`, info.v2signals.join(' | '));
    }
    return info;
  }
